#!/usr/bin/env bash
# Import dashboard-template.json into a Dynatrace tenant.
#
...
#
# Token overview for both credential types: README.md in this directory.
#
# Usage: ./import_dashboard.sh [dashboard.json] [--name "..."]
#
...
DASHBOARD_FILE="${1:-$HERE/dashboard-template.json}"
...
if [ -z "${DT_ENVIRONMENT_ID:-}" ] || \
  ...
  if [ -f "$HERE/.env" ]; then
    echo "Loading credentials from $HERE/.env"
...
# which reads like a scope problem and is not one — same trap as OTLP ingest,
# which only the .live. host serves.
...
print(t if t else 'ERR:'+str(d.get('error_description') or d.get('error','unknown')))" \
  ...
  fail "no dashboard credential found. Either:
       ...
       Token overview: README.md in this directory."
fi
...
try:
  ...
  401|403)
    ...
    echo "  See the token table in README.md." >&2
    exit 1 ;;
  409)
