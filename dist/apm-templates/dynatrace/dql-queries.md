...
For *which* metrics exist, see Apollo's
[standard metric instruments](https://www.apollographql.com/docs/graphos/routing/observability/router-telemetry-otel/enabling-telemetry/standard-instruments)
reference. This document is about reading them in Dynatrace.
...
GraphOS Studio reports milliseconds — three scales for one quantity.
...
**1. Enable it.** In
[`instruments.router.yaml`](instruments.router.yaml), under
...
Check the name and type carefully before restarting: the router accepts naming
and type mistakes silently, and a custom instrument that squats on the `http.*`
or `apollo.*` namespaces will collide with the standard ones. See Apollo's
[instruments documentation](https://www.apollographql.com/docs/graphos/routing/observability/router-telemetry-otel/enabling-telemetry/instruments)
for the full custom-instrument schema.
...
interval plus ingest lag, usually under a minute.

Some metrics never arrive because Dynatrace rejects the type at ingest, which leaves
no error in the router log — UpDownCounters under delta temporality are the main
case (see shape E above).

**3. Chart it** using shape B.

**4. Place it.** Add a tile in the Dynatrace dashboard editor (or edit your copy of
`dashboard-template.json`) using the query from step 3. Note that re-importing this
template later replaces the same-named dashboard, so keep customized copies under a
different name (`DASHBOARD_NAME=...`).
...
carries several — and GraphOS Studio counts reported operations and is sampled.
Check batching and samplers before treating a gap as a defect.
